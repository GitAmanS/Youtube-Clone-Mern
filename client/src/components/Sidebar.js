...
const Sidebar = ({ isCollapsed, toggleSidebar }) => {
  ...
  return (
        ...
        }   py-2 px-4 flex flex-col  space-y-4 transition-all duration-300`}
        >
        ...
        {/* Sidebar Links */}
        <nav className={`flex flex-col space-y-3 my-3 justify-center ${isCollapsed && "items-center"} `}>
            <a onClick={()=>{navigateTo("/")}} className={`flex flex-row bg-white bg-opacity-10 ${isCollapsed ? "flex-col items-center justify-center   " : "flex-row items-start justify-start mr-6 "}    text-white rounded-lg p-1.5 m-1 mx-2 px-2.5 `}>
            <svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7 text-white" fill="currentColor" viewBox="0 0 24 24">
                <path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z" />
            </svg>
            <span className={`text-[10px] py-1 ${isCollapsed ? " block":"hidden"}`}>Home</span>
            <span  className={`text-sm py-1 pl-4 font-semibold ${isCollapsed ? " hidden":"block"}`}>Home</span>
            </a>
